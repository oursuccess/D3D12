//copy of TexColumnsApp of Frack Luna, ch09. 
...
private:
	...
	//第9章。添加回创建描述符堆的方法。因为SRV只能使用描述符表
	void BuildDescriptorHeaps();
	void BuildShadersAndInputLayout();
	void BuildShapesGeometry();
	//在第9章里，这个方法被整合到BuildShapesGeometry中了，但是我没有跟随
...
private:
	...
	//在原本的代码里 这里是mPSOs，但是我继承自LitColumn，因此这里没有改
...
TexColumns::TexColumns(HINSTANCE hInstance) : D3DApp(hInstance)
{
}
...
void TexColumns::BuildDescriptorHeaps()
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE hDescriptors(mSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
	...
	md3dDevice->CreateShaderResourceView(bricksTex.Get(), &srvDesc, hDescriptors);

	//偏移
	hDescriptors.Offset(1, mCbvSrvDescriptorSize);

	srvDesc.Format = stoneTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = stoneTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(stoneTex.Get(), &srvDesc, hDescriptors);
	...
	md3dDevice->CreateShaderResourceView(tileTex.Get(), &srvDesc, hDescriptors);
}
...
void TexColumns::BuildShapesGeometry()
{
...
//ch09没有骷髅头了
void TexColumns::BuildSkullGeometry()
{
...
void TexColumns::BuildRenderItems()
{
	...
	*/
	...
	for(int i = 0; i < 5; ++i)
	{
		...
		//ch09, shphere现在的Transform改成单位矩阵了
...
void TexColumns::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); ++i)
	{
		...
		//ch09,我们添加贴图的描述符表，并将物体的常量缓冲区和材质的常量缓冲区后移
