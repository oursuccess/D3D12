...
#pragma region Quiz0804
	...
	*/
	//我们将光源修改为在每个球体中心一个。 因此我们需要知道球体中心们的位置
	//我们先添加一个全局的方向光
	//mMainPassCB.Lights[0].Direction = {}
	XMStoreFloat3(&mMainPassCB.Lights[0].Direction, -MathHelper::SphericalToCartesian(1.0f, mSunTheta, mSunPhi));
	mMainPassCB.Lights[0].Strength = { 0.5f, 0.5f, 0.5f };
	UINT32 iMax = min(MaxLights, mLightPoss.size()) - 1;
