//copy of LitWaves by Frank Luna, Ch08
...
void LitWaves::Draw(const GameTimer& gt)
{
	...
	//第8章。因为现在1被MaterialCB用了，因此这里从1变成了2
...
void LitWaves::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for(auto& e : mAllRitems)
	{
		...
		if(e->NumFramesDirty > 0)
		{
			...
			//在这里添加texTransform的选项。第8章暂时还用不到这个
...
void LitWaves::UpdateMainPassCB(const GameTimer& gt)
{
	...
	//第8章。更新光照方向。这里只有一个光源了
	XMVECTOR lightDir = -MathHelper::SphericalToCartesian(1.0f, mSunTheta, mSunPhi);
	XMStoreFloat3(&mMainPassCB.Lights[0].Direction, lightDir);
#pragma region Quiz0801
	//mMainPassCB.Lights[0].Strength = { 1.0f, 1.0f, 0.9f };
	mMainPassCB.Lights[0].Strength = { MathHelper::Clamp(sin(gt.TotalTime()), 0.0f, 1.0f), 0.0f, 0.0f};
#pragma endregion
...
void LitWaves::BuildRootSignature()
{
	...
	//第7章的陆地与波浪程序本来就是直接绑定了根描述符。我们在这里需要增加一个新的(Material的)。因此，数组长度从2变为3
