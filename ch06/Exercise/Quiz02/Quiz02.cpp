...
#pragma region Quiz0602
	...
	std::unique_ptr<quiz0602::MeshGeometry> mBoxGeo = nullptr;
#pragma endregion
...
#pragma region Quiz0602
	...
	*/
	...
	mBoxGeo = std::make_unique<quiz0602::MeshGeometry>();
	mBoxGeo->Name = "boxGeo";

	const UINT pbByteSize = (UINT)positions.size() * sizeof(VPosData);
	ThrowIfFailed(D3DCreateBlob(pbByteSize, &mBoxGeo->PositionBufferCPU));
	CopyMemory(mBoxGeo->PositionBufferCPU->GetBufferPointer(), positions.data(), pbByteSize);

	mBoxGeo->PositionBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(), mCommandList.Get(), positions.data(), pbByteSize, mBoxGeo->PositionBufferUploader);
