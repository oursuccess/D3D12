//Solution for quiz2011, by Je
//要实现点光源阴影，我们仿照动态立方体图的构建方式, 构建从点光源到上下左右前后六个方向的阴影图， 然后在绘制物体时，对该阴影立方体图进行采样即可
...
#pragma region Quiz2011
    //添加绘制每个面的阴影的方法
    void UpdateShadowMapFacePassCBs(const GameTimer& gt);
...
private:
	...
	Camera mCamera;
#pragma region Quiz2011
    //我们需要添加6个用于绘制点光源阴影的相机
    Camera mPointLightShadowMapCameras[6];
...
void ShadowMapApp::UpdateShadowMapFacePassCBs(const GameTimer& gt)
{
    for (int i = 0; i < 6; ++i)
    {
        ...
        XMMATRIX view = mPointLightShadowMapCameras[i].GetView();
        XMMATRIX proj = mPointLightShadowMapCameras[i].GetProj();
        ...
        //再次重复一遍, 之所以要在这里转置, 是因为在dx中是行优先, 但是在hlsl中默认变成了列优先! 因此为了在Gpu中能取回原本的正常的矩阵, 我们必须首先转置!
        XMStoreFloat4x4(&shadowFacePassCB.View, XMMatrixTranspose(view));
        XMStoreFloat4x4(&shadowFacePassCB.Proj, XMMatrixTranspose(proj));
        XMStoreFloat4x4(&shadowFacePassCB.ViewProj, XMMatrixTranspose(viewProj));
        XMStoreFloat4x4(&shadowFacePassCB.InvView, XMMatrixTranspose(invView));
...
void ShadowMapApp::BuildShadowFaceCamera(float x, float y, float z)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        mPointLightShadowMapCameras[i].LookAt(center, targets[i], ups[i]);
        mPointLightShadowMapCameras[i].SetLens(0.5f * XM_PI, 1.0f, 0.1f, 1000.0f);
        mPointLightShadowMapCameras[i].UpdateViewMatrix();
    }
}
