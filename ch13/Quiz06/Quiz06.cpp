//solution for Quiz1306, by Je
//copy from SobelFilter
...
WavesCS::WavesCS(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}
...
void WavesCS::Draw(const GameTimer& gt)
{
...
void WavesCS::OnKeyboardInput(const GameTimer& gt)
{
}

void WavesCS::UpdateCamera(const GameTimer& gt)
{
...
void WavesCS::AnimateMaterials(const GameTimer& gt)
{
	//将水的材质贴图进行滚动
...
void WavesCS::UpdateObjectCBs(const GameTimer& gt)
{
...
void WavesCS::UpdateMaterialCBs(const GameTimer& gt)
{
...
void WavesCS::UpdateMainPassCB(const GameTimer& gt)
{
...
#pragma region Quiz1305
//我们直接从UpdateWaves里找找能用的
void WavesCS::UpdateWavesGPU(const GameTimer& gt)
{
	...
	if ((mTimer.TotalTime() - t_base) >= 0.25f)
	{
		...
		//disturb现在需要加上cmdList、根签名和PSO了
		mWaves->Disturb(mCommandList.Get(), mWavesRootSignature.Get(), mPSOs["wavesDisturb"].Get(), i, j, r);
	}

	//Update同样也有了cmdList、根签名和PSO
	mWaves->Update(gt, mCommandList.Get(), mWavesRootSignature.Get(), mPSOs["wavesDisturb"].Get());
...
void WavesCS::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
